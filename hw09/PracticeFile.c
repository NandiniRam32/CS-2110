...
void empty_list(LinkedList *list)
{
    ...
    list->size = 0;
    return;
    //what other functions are we supposed to be using?
}
...
int num_passing_all_classes(LinkedList *list, int *dataOut)
{
    ...
    *dataOut = 0;
    ...
    while (index < list->size) {
        if (currentNode->data->type == STUDENT) {
            Student student = currentNode->data->data.student;
            int passing = 1;
            if (student.num_classes > 0) {
                int dex = 0;
                while (dex < student.num_classes) {
...
int get_average_salary(LinkedList *list, double *dataOut)
{
    ...
    *dataOut = 0;
    ...
    while (index < list->size) {
        if (currentNode->data->type == INSTRUCTOR) {
            Instructor instructor = currentNode->data->data.instructor;
