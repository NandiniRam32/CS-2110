...
metadata_t *find_right(metadata_t *freed_block) {
    ...
    while (curr) {
        if ((uintptr_t) ((uint8_t*) (curr)) == (uintptr_t) freed_block + (uintptr_t) freed_block->size + sizeof(metadata_t)) {
            return curr;
        }
        curr = curr->next;
    }
 ...
 */
 metadata_t *split_block(metadata_t *block, size_t size) {
    ...
    //Step 1
    size_t memory = size + sizeof(metadata_t);
    //Step 2
    metadata_t * address = (metadata_t*) ((uint8_t*) block + block->size - size);
    //Step 3
    address->size = size;
    block->size -= memory;
    //Step 4
 ...
 */
 metadata_t *find_best_fit(size_t size) {
    ...
    while (current) {
        ...
        if (current->size > size && bestFitBlock == NULL) {
            bestFitBlock = current;
        }
        if (current->size > size && current->size < bestFitBlock->size) {
            bestFitBlock = current;
        }
        current = current->next;
    }
...
void *my_malloc(size_t size) {
    ...
    if (block == NULL) {
        ...
        if ((size_t) block == negative) {
            ...
            block->next = NULL;
            add_to_addr_list(block);
            if (find_left(block) != NULL) {
                block = find_left(block);
                merge(block, find_right(block));
            }
            //add_to_addr_list(block);
        }
    }
    ...
    if (block->size - size < MIN_BLOCK_SIZE || block->size == size) {
...
void *my_realloc(void *ptr, size_t size) {
    ...
    metadata_t * allocation = (metadata_t *) ((char *) ptr - sizeof(metadata_t));
    size_t minimum = size;
    ...
    void * newPtr = memcpy(newBlock, ptr, minimum);
...
void *my_calloc(size_t nmemb, size_t size) {
    ...
    void * newBlock = my_malloc(newSize);
    if (newBlock == NULL) {
        return NULL;
    }
