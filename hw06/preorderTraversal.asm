...
PREORDER_TRAVERSAL ;; Do not change this label! Treat this as like the name of the function in a function header
    ...
    LDR   R1, R5, 5
    ;;Load value of index into R2
    LDR   R2, R5, 6

    ;;If statement
IF  ADD   R0, R0, 0
    ;;If root is negative or positive, we jump to ENDIF
    BRnp ENDIF
    ;;Otherwise, we store index in R5 offset 3
    STR   R2, R5, 3
    ;;Branch to Teardown
    BR TEARDOWN
    ;;ENDIF
    ...
    ;;First subroutine call Registers:
    ;;address of root.left <-- R0
    AND   R0, R0, 0
    LDR   R0, R5, 4
    ADD   R3, R0, 1
    LDR   R3, R3, 0
    ...
    ;;Load return value into index
    LDR   R2, R6, 0
    ;;Pop out 4 values
    ADD   R6, R6, 4

    ;;First subroutine call Registers:
    ;;address of root.right <-- R0
    LDR   R0, R5, 4
    ADD   R3, R0, 2
    LDR   R3, R3, 0
    ;;address of arr <-- R1
